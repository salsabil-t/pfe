...
export default function HomeScreen() {
  ...
  // ─── FIXED: fetchTodaySchedule ────────────────────────────────────────────
  // Key fixes:
  //   1. Table name: medication_log → medication_logs
  //   2. Match logic: l.take_id === t.id (wrong column)
  //            → l.medication_id === med.id && l.scheduled_time === t.time
  //   3. Fetch all takes + logs ONCE outside the loop (better performance)
  // ──────────────────────────────────────────────────────────────────────────
  const fetchTodaySchedule = async (userId) => {
    const todayStr = new Date().toISOString().split('T')[0]; // "2025-04-17"

    // 1. Get all meds for this user
    ...
    // 2. Get ALL takes for these meds at once
    const { data: allTakes } = await supabase
      .from("medication takes")
      .select("*")
      .in("medication_id", medIds);

    // 3. Get today's logs — match by taken_at date range
    //    (medication_logs has no scheduled_date column, so we use taken_at)
    ...
    for (const med of meds) {
      ...
      if (med.schedule_type === "consecutive") {
        ...
        // "specific" type: check medication_dates
      ...
      if (activeToday) {
        const medTakes = allTakes?.filter(t => t.medication_id === med.id) || [];

        medTakes.forEach(t => {
          // ✅ FIXED: match on medication_id + scheduled_time (not take_id)
          const isTaken = todayLogs?.some(
            l =>
              l.medication_id === med.id &&
              l.scheduled_time === t.time &&
              l.status === 'taken'
          ) ?? false;
          ...
          schedule.push({
            ...
            dose: t.dose,
            taken: isTaken,
            // pending = scheduled time is still in the future
  ...
  // ─── STOCK (unchanged logic, fixed table name) ────────────────────────────
  ...
  const deleteStockItem = async (id) => {
    ...
    if (!error) {
      const { data: { user } } = await supabase.auth.getUser();
      fetchMedicationStock(user.id);
    }
  };

  // Returns true if the scheduled time hasn't passed yet today
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
          ...
          <TouchableOpacity style={styles.patientCard} onPress={openPatientModal}>
            ...
            <View style={styles.infoRow}>
              ...
              <Text style={styles.value}>
                {patientData.age ? `${patientData.age} years old` : "N/A"}
              </Text>
            </View>
            <View style={styles.infoRow}>
        ...
        <View style={styles.sectionContainer}>
          ...
          {loading ? (
            <ActivityIndicator color="#7DD1E0" style={{ marginTop: 10 }} />
              ...
              {todaySchedule.map((item) => (
                <View key={item.id} style={styles.scheduleCard}>
                  <View style={styles.iconContainer}>
                    {item.taken ? (
                      // ✅ Green  — patient confirmed they took it
                      <Ionicons name="checkmark-circle" size={32} color="#27ae60" />
                    ) : item.pending ? (
                      // 🕐 Yellow — scheduled time hasn't arrived yet
                      <Ionicons name="time" size={32} color="#f39c12" />
                    ) : (
                      // ❌ Red    — time has passed and it wasn't taken
                  ...
                  <View style={styles.stockTextGroup}>
                    ...
                    <Text
                      style={[
                        ...
                        { color: item.daysRemaining <= 0 ? '#e74c3c' : '#555' },
                      ]}
                    >
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TextInput
              style={styles.input}
              placeholder="Disease"
              value={editPatientDisease}
              onChangeText={setEditPatientDisease}
            />
            <View style={styles.modalButtons}>
              ...
              <TouchableOpacity style={styles.saveBtn} onPress={savePatientProfile}>
...
const styles = StyleSheet.create({
  ...
  patientCard:        { backgroundColor: 'rgba(255,255,255,0.9)', borderRadius: 33, padding: 18 },
  infoRow:            { flexDirection: 'row', marginBottom: 6 },
  label:              { width: 75, fontWeight: 'bold', color: '#0b4f5c' },
  value:              { color: '#0b4f5c', flex: 1, fontSize: 17, fontWeight: '600' },
  scheduleCard:       { backgroundColor: '#fff', borderRadius: 33, padding: 15, marginRight: 12, width: 130, alignItems: 'center' },
